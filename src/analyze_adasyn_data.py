...
def main() -> None:
    ...
    synthetic_success["Type"] = "Synthetic (ADASYN)"
    ...
    summary.to_csv(RESULTS_DIR / "adasyn_synthetic_summary.csv", index=False)
    ...
    for index, feature in enumerate(FEATURES, 1):
        ...
        plt.title(f"{feature} - Original vs ADASYN")
        plt.grid(axis="y", linestyle="--", alpha=0.5)

    plt.tight_layout()
    figure_path = FIGURES_DIR / "adasyn_istatistik_analizi.png"
    plt.savefig(figure_path, dpi=300)
    plt.close()

    print("ADASYN analysis completed.")
    print(f"Summary: {RESULTS_DIR / 'adasyn_synthetic_summary.csv'}")
